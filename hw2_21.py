...
reward = functools.partial(_reward, fire_states=fire_states, goal_states=goal_states, fire_value=fire_value,
                           goal_value=goal_value, travel_value=travel_value)  ## fix all args except state
transition_function = functools.partial(_transition_function, max_rows=max_rows, max_columns=max_columns,
                                        goal_states=goal_states, action_set=action_set)
...
def probability_function(state, action, next_state, w):
    ...
    if next_state == next_state_det:
        ...
        if next_state in next_state_list:
            prob = w / len(next_state_list) #random
        else:
            prob = 0 # no chance
    return prob
...
def plot_value_function(iteration, t):
    ...
    plt.subplot(1, 2, 1)
    plt.imshow(Vs[iteration][0], origin='lower', extent=[0, max_columns, 0, max_rows], cmap='viridis',
    ...
    plt.subplot(1, 2, 2)
    plt.imshow(storm_strength, origin='lower', extent=[0, max_columns, 0, max_rows], cmap='viridis',
               interpolation='nearest')
